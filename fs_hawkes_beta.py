...
class FSHawkesBeta:
    ...
    def Phi_n_g_States_g(self, points_hawkes, states, T, num_grid):
        r"""
        Evaluate \Phi(t) on all observed points and grid nodes, and the state of grid nodes
        ...
        :rtype: number_of_dimensions*N_i*(number_of_dimensions*number_of_basis+1), num_grid*(number_of_dimensions*number_of_basis+1), 1-D numpy array
        :return: list of \Phi(t_n), \Phi(t_grid), state on grid node
        """
        ...
        Phi_g = np.zeros((num_grid, self.number_of_dimensions*self.number_of_basis+1))
        for g in range(num_grid):
            Phi_g[g] = self.Phi_t(np.linspace(0,T,num_grid)[g],points_hawkes)
        states_g = [states[sum(~(points_hawkes_sum>=t))] for t in np.linspace(0,T,num_grid)]
    ...
    def loglikelyhood(self, W, lamda, Phi_n, states_n, Phi_g, states_g, points_hawkes, T):
        r"""
    ...
    def Gibbs(self, points_hawkes, states, states_n, points_hawkes_test, states_test, states_n_test, \
        T, T_test, var_prior, alpha_prior, num_grid, num_grid_test, num_iter, initial_W = None): 
        r"""
        ...
        :type var_prior: float
        :param var_prior: the variance \sigma^2 of Gaussian prior
        :type alpha_prior: float
        :param alpha_prior: the parameter of Dirichlet prior, [alpha_prior,...,alpha_prior]
        :type num_grid: int
        :param num_grid: the number of grid nodes on [0,T]
        ...
        :rtype: numpy array
        :return: the samples of lamda_ub (lamda) and weight (W), the training (logl) and test log-likelihood (logl_test)
        by those samples. 
        """
        ...
        Phi_n,Phi_g,states_g=self.Phi_n_g_States_g(points_hawkes, states, T, num_grid)
        ...
        logl=[]
        logl_test=[]
        Phi_n_test,Phi_g_test,states_g_test=self.Phi_n_g_States_g(points_hawkes_test,states_test,T_test,num_grid_test)
        
        for ite in range(num_iter):
            for d in range(self.number_of_dimensions):
                # sample P_d
                for z in range(self.number_of_states):
                    P[d][z]=dirichlet.rvs(counts[d][z]+alpha_prior)
                
                # sample w_n
                for n in range(N[d]):    
                    w_n[d][n]=self.PG(1,W[states_n[d][n]][d].dot(Phi_n[d][n]))
                ...
                w_m[d]=np.array([self.PG(1,W[states_m[d][m]][d].dot(Phi_m[d][m])) for m in range(len(t_m[d]))])
                ...
                for z in range(self.number_of_states):
                    ...
                    Sigma_W_z=np.linalg.inv((Phi_z.T).dot(Sigma_z_inv).dot(Phi_z)+1/var_prior*np.eye(self.number_of_dimensions*self.number_of_basis+1))
                    mean_W_z=Sigma_W_z.dot(Phi_z.T).dot(v_z)
                    W[z][d]=multivariate_normal(mean=mean_W_z,cov=Sigma_W_z).rvs()
    #         w_n_list.append(copy.deepcopy(w_n))
    #         t_m_list.append(copy.deepcopy(t_m))
    #         w_m_list.append(copy.deepcopy(w_m))
            lamda_list.append(lamda.copy())
            W_list.append(W.copy())
            P_list.append(P.copy())
            
            # compute the loglikelihood
            logl.append(self.loglikelyhood(W,lamda,Phi_n,states_n,Phi_g,states_g,points_hawkes,T))
            logl_test.append(self.loglikelyhood(W,lamda,Phi_n_test,states_n_test,Phi_g_test,states_g_test,points_hawkes_test,T_test))
    ...
    #     a=Phi[d][n].dot(mean_W[z][d])
        c=np.sqrt((Phi[d][n].dot(mean_W[z][d]))**2+(Phi[d][n].T).dot(cov_W[z][d]).dot(Phi[d][n]))
        return c
    ...
    def a_c_predict_gq(z,d,n,Phi,mean_W,cov_W):
        ...
        c=np.sqrt((Phi[n].dot(mean_W[z][d]))**2+(Phi[n].T).dot(cov_W[z][d]).dot(Phi[n]))
        return a,c
    ...
    def loglikelyhood_vi(self, W, lamda, Phi_n, states_n, Phi_gq, states_gq, w_gq, points_hawkes):
        r"""
        Evaluate the log-likelihood for the given timestamps in the iterative update of mean-field algorithm
        ...
        :param w_gq: the gaussian quadrature weight of each gaussian quadrature node
        :type points_hawkes: list
        :param points_hawkes: the timestamps when events occur on each dimension
        :rtype: float
        :return: the log-likelihood for the given timestamps
        ...
        for i in range(self.number_of_dimensions):
            ...
            for n in range(N_i):
                logl+=np.log(expit(W[states_n[i][n]][i].dot(Phi_n[i][n])))+np.log(lamda[i])
            for n in range(N_gq):
                logl-=w_gq[n]*lamda[i]*expit(W[states_gq[n]][i].dot(Phi_gq[n]))
        return logl

    def MeanField(self, points_hawkes, states, states_n, points_hawkes_test, states_test, states_n_test, \
        T, T_test, var_prior, alpha_prior, num_gq_seg, num_gq_seg_test, num_iter, initial_W_mean = None):
        r"""
        ...
        :param alpha_prior: the parameter of Dirichlet prior, [alpha_prior,...,alpha_prior]
        :type num_gq_seg: int
        :param num_gq_seg: the number of gaussian quadrature nodes on each state-segment for training data
        :type num_gq_seg_test: int
        :param num_gq_seg_test: the number of gaussian quadrature nodes on each state-segment for test data
        :type num_iter: int
        ...
        :rtype: numpy array
        :return: the parameter of lamda_ub posterior, the parameter of weight posterior, the parameter of transition matrix posterior
        ...
        Phi_n=[np.zeros((N[i],self.number_of_dimensions*self.number_of_basis+1)) for i in range(self.number_of_dimensions)] # precompute Phi(t) on observations, D*N_i*(DB+1)
        for i in range(self.number_of_dimensions):
            for n in range(N[i]):
                Phi_n[i][n]=self.Phi_t(points_hawkes[i][n],points_hawkes)
        
        p_gq,w_gq,states_gq=self.gq_points_weights_states_T(points_hawkes,states,T,num_gq_seg)
        num_gq=len(p_gq)
        a_gq=np.zeros((self.number_of_dimensions,num_gq))
        c_gq=np.zeros((self.number_of_dimensions,num_gq))
        Phi_gq=np.zeros((num_gq,self.number_of_dimensions*self.number_of_basis+1)) #precompute the Phi_gq on gauss quadureture points
        for n in range(num_gq):
            Phi_gq[n]=self.Phi_t(p_gq[n],points_hawkes)
        
        logl=[]
        logl_test=[]

        N_test=np.array([len(points_hawkes_test[i]) for i in range(self.number_of_dimensions)]) # of points on each dimension 
        Phi_n_test=[np.zeros((N_test[i],self.number_of_dimensions*self.number_of_basis+1)) for i in range(self.number_of_dimensions)]
        for i in range(self.number_of_dimensions):
            for n in range(N_test[i]):
                Phi_n_test[i][n]=self.Phi_t(points_hawkes_test[i][n],points_hawkes_test)
        p_gq_test,w_gq_test,states_gq_test=self.gq_points_weights_states_T(points_hawkes_test,states_test,T_test,num_gq_seg_test)
        num_gq_test=len(p_gq_test)
        Phi_gq_test=np.zeros((num_gq_test,self.number_of_dimensions*self.number_of_basis+1))
        for n in range(num_gq_test):
            Phi_gq_test[n]=self.Phi_t(p_gq_test[n],points_hawkes_test)
        ...
        for ite in range(num_iter):
            for i in range(self.number_of_dimensions):
                ...
                for z in range(self.number_of_states):
                    ...
                    cov_W[z][i]=np.linalg.inv(int_A+1/var_prior*np.eye(self.number_of_dimensions*self.number_of_basis+1))
                    mean_W[z][i]=cov_W[z][i].dot(int_B)
            logl.append(self.loglikelyhood_vi(mean_W,alpha/T,Phi_n,states_n,Phi_gq,states_gq,w_gq,points_hawkes))
            logl_test.append(self.loglikelyhood_vi(mean_W,alpha/T,Phi_n_test,states_n_test,Phi_gq_test,states_gq_test,w_gq_test,points_hawkes_test))
        return alpha, mean_W, cov_W, counts+alpha_prior, logl, logl_test
